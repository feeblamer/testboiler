...
- сохраняет состояние последней успешной установки внутри `.venv`;
- при повторном вызове не запускает установку заново, если `config.yml` и `requirements.txt` не менялись.

Команда `testboiler run`:
- использует уже существующий `.venv`;
- не выполняет установку пакетов повторно;
- проверяет, что `.venv` не устарел относительно `config.yml` и `requirements.txt`;
...
- `testboiler install --force` — принудительно переустанавливает зависимости в `.venv`;
